...
public class PriceIntervalConverter {
    ...
    public boolean convert(File sourceDirectory, File destinationFile, double maxPercentDiff) {
        ...
        // Convert!
        try (IPriceCandleStreamSource files =
                new PriceCandleDirectoryStreamSource(sourceDirectory, parser).setLineFilter(filter)) {
            try (IPriceCandleStreamSource intervalSource = new PriceCandleIntervalStreamSource(files, interval)) {
                try (IPriceCandleStreamSource checkerSource = new PriceCandleCheckerStreamSource(
                        intervalSource, new PriceCandleSanityChecker(maxPercentDiff))) {
                    try (IPriceCandleStreamSink sink =
                            new PriceCandleLineStreamSink(new FileLineSink(destinationFile))) {
