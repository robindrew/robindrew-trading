...
public class PriceCandles {
    ...
    public static double getWeighted(IPriceCandle candle) {
        ...
        return (candle.getMidHighPrice()
                        + candle.getMidLowPrice()
                        + candle.getMidClosePrice()
                        + candle.getMidClosePrice())
                / 4.0;
    }
    ...
    public static IPriceCandleStreamSource filterByDates(
            IPriceCandleStreamSource source, LocalDateTime from, LocalDateTime to) {
        return new PriceCandleFilteredStreamSource(source, new PriceCandleDateTimeFilter(from, to));
    }

    public static IPriceCandleStreamSource filterByDates(
            IPriceCandleStreamSource source, LocalDate from, LocalDate to) {
